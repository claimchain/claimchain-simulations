...
def parse_mail(dirpath, filename):
    ...
    try:
        ...
        logging.debug('Found email without date, will ignore')
        return
    ...
    try:
        ...
        mID = int(hashlib.sha1(
            aux_string.lower().encode('utf-8')).hexdigest(), 16)
    except:
...
def process_enron(root_folder='data/enron/maildir/', parsed_folder='data/enron/parsed/'):
    ...
    mail_list = []
    seen_msgs = set()  # Create list to detect duplicate messages

    logging.info('Parse outgoing messages...')
    progress = tqdm(os.listdir(root_folder))
    for username in progress:
        progress.set_description('Parsing outbox folders of user %-15s' % username)
        ...
        if counter < 20:
            logging.debug('User %s does not have enough sent messages', user_folder)
            continue
        '''
        ...
        for folder in sent_folders:
            for dirpath, _, filenames in os.walk(
                ...
                for filename in filenames:
                    ...
                    seen_msgs.add(mID)  # Update the duplicates set
                    ...
                    if len(t_set) == 0:
                        ...
                        logging.debug(
                            'Found email without public name@domain.xz recipients')
                        continue
        ...
        if len(from_headers_set) == 0:
            logging.debug('User %s had no sent emails' % (username))
            continue
        if len(from_headers_set) > 1:
            logging.debug('User %s is using multiple From headers:%s' %
                         (username, from_headers_set))

        most_used_from_header = max(
            from_headers_set, key=from_headers_list.count)

        # Log the social graph of the user
        social[most_used_from_header] = {
                'user': username,
                'friends': r_set,
                'num_of_friends': len(r_set),
                'from_headers_set': from_headers_set
        }

    logging.info('Parse incoming messages...')
    progress = tqdm(os.listdir(root_folder))
    for username in progress:
        progress.set_description('Parsing inbox folders of user %-15s ' % username)
        ...
        for folder in received_folders:
            folder_path = root_folder + username + '/' + folder
    ...
    pickle.dump(emails_per_num_of_recipients, open(
        parsed_folder + 'recipients.pkl', 'wb'))
    pickle.dump(mail_list, open(parsed_folder + 'replay_log.pkl', 'wb'))

    return (social, emails_per_num_of_recipients, cnt_msgs,
            cnt_msgs_no_recipients, cnt_msgs_dup, cnt_msgs_invalid)


def main():
    _, _, cnt_msgs, cnt_msgs_no_recipients, cnt_msgs_dup, cnt_msgs_invalid = process_enron()
    logging.info(
        'Parsed %s messages, discarded %s because they had no valid recipient '
        'email address, %s because they were '
